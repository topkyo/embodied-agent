...
| `BLOCKED · Sim Matrix *`        | 报告过期（默认 7 天）或未签名。生成：`EVAL_EVIDENCE_SECRET=…`（与 API 用同一值） + `cd the production checkout && set -a; source .env.vps.local; set +a` 后跑 `SIM_MATRIX_SLICE=core\|wechat\|negative npx tsx scripts/simulate-user-matrix.ts`。生产 VPS 完整步骤 + cron 续签建议见 `deploy/vps/TROUBLESHOOTING.zh.md` §1.2。注意：**签报告与跑 API 须同一 secret** 才 `attested=true` |
...
客户 deployment 工作台对外前，至少确认下列项（变量 → 消费方 → 缺失行为见 [`docs/operations/env-keys.zh.md`](env-keys.zh.md) §5；VPS 部署见 `deploy/vps/README.zh.md`）。
