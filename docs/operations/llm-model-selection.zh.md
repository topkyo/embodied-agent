...
> 依据 [DeepSeek 定价与模型说明](https://api-docs.deepseek.com/quick_start/pricing)（2026-04 更新）。
...
运行数据目录已 `.gitignore`，勿提交密钥。部署栈见 [`deploy/vps/README.zh.md`](../../deploy/vps/README.zh.md)；本地一键栈可用 `npm run stack:up`（Compose，与 VPS 部署无关）。
