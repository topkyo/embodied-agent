...
在唯一活跃仓库 `topkyo/embodied-agent` 内，将代码库从「数字农场默认叙事」升级为「具身Agent 平台 + greenhouse Domain Pack」。Web 重构（v3.1 React）与 Phase C Deployment 模型均已完成；本 spec 记录**叙事层、包名、Domain Pack 抽象**决策。实施计划见 （实施计划未纳入本公开快照）。
